...
public class Global{
    static var clientID = "e5ac0d6c6c1291353bd696af5ad24691"
    static var clientSecret = "c1f507adb3e42d72d953b2928e52ef96aaab0a355139cc84911b6bf1accfa0b4"
    static var TeacherContractName = "epf.teacher"
    static var BasicContractName = "epf.basic"
...
func GetCommonConnect(dsns:String,contract:String) -> Connection{
    
    var retVal : Connection!
    
    dispatch_sync(Global.LockQueue) {
        
        let key = dsns + "_" + contract
        
        if Global.SchoolConnector[key] == nil{
            
            var err: DSFault!
            
            Global.SchoolConnector[key] = Connection()
            
            Global.SchoolConnector[key]!.connect(dsns, contract, SecurityToken.createOAuthToken(Global.AccessToken), &err)
        ...
        retVal = Global.SchoolConnector[key]
...
func GetImageFromBase64(baseStr:String) -> UIImage{
    
    let decodedData = NSData(base64EncodedString: baseStr, options: NSDataBase64DecodingOptions(rawValue: 0))
    
    if let data = decodedData{
        if let retVal = UIImage(data: data){
            return retVal
        }
    }
